...
`Math.random()` / `Date.now()` / `fetch` 等を直接呼び出さず、`Random` / `Clock` 等のインターフェースを引数で受け取ります (例: `src/domain/random.ts`)。これによりテストが決定的になり、CI が flaky にならず、ralph-loop による自律開発が安定動作します。

### immutable な状態

ゲーム状態は `readonly` フィールドで宣言し、状態更新関数 (例: `src/domain/combat.ts` の `step`) は新しいオブジェクトを返します。時系列バグの防止 + テスト容易性。

### テストとセットでコミット

`src/domain/` に関数を追加・変更したら、必ず `tests/domain/<name>.test.ts` に対応テストを書き、**同じコミットに含めます**。ralph-loop の Step 4 で `git add -A && git commit` で 1 コミット化。
